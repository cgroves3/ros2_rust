...
impl ActionClientHandle {
    ...
    pub fn action_server_is_ready(&self) -> bool {
        ...
        let handle = &*self.rcl_action_client_mtx.lock();
        let node_handle = &*self.rcl_node_mtx.lock();
        unsafe {
    ...
    T: rosidl_runtime_rs::Action,
{
    ...
    /// rclcpp uses a std::map, which is ordered, but it doesn't get the minimum/maximum key, so instead a hashmap is used
    pending_result_responses: Arc<Mutex<HashMap<i64, T::Goal>>>,
    /// rclcpp uses a std::map, which is ordered, but it doesn't get the minimum/maximum key, so instead a hashmap is used
    pending_cancel_responses: Arc<Mutex<HashMap<i64, T::Goal>>>
