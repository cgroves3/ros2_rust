...
use crate::action::{GoalUUID, ServerGoalHandle, ServerGoalHandleHandle};
...
impl ActionServerHandle {
    ...
    pub fn publish_status(&self) -> Result<(), RclrsError> {
        ...
        let goal_status_array_c = crate::action::types::GoalStatusArrayHandle::new(rcl_goal_status_array);
    ...
    T: rosidl_runtime_rs::Action,
{
    ...
    > {
        ...
        let unwrapped_handle = self.handle.lock().unwrap();
            ...
            let server_goal_handle_mtx = Arc::new(
                Mutex::new(
                    ServerGoalHandle::<T>::new(
                        ...
                        uuid,
                        self.handle.clone(),
                        self.goal_results.clone(),
                        self.result_requests.clone()
                    )
                )
            );
    ...
    pub fn accept_new_goal(&self, goal_info_handle: GoalInfoHandle) -> ServerGoalHandleHandle {
        let unwrapped_handle = self.handle.lock().unwrap();
        let mut_handle = &mut *(unwrapped_handle.lock());
    ...
    pub fn execute_result_request_received(&self) -> Result<(), RclrsError> {
        ...
        if !goal_exists {
            ...
            {
                let rmw_message = 
                    <<<T as Action>::GetResult as GetResultService>::Response as Message>::into_rmw_message(result_response.into_cow());
                unsafe {
                    rcl_action_send_result_response(
                        handle,
                        &mut req_id,
    ...
    pub fn call_handle_cancel_callback(&self, goal_uuid: GoalUUID) -> CancelResponse {
        if let Some(goal_handle) = { self.goal_handles.lock().unwrap() }.get(&goal_uuid) {
            if let Some(goal_handle_arc) = Weak::upgrade(goal_handle) {
